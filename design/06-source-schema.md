...
| 3 | 도로명코드 | `road_code_total.txt` | 20 | 전국 한 벌 |
| 4 | 도로명 | `TN_SPRD_RDNM.txt` | 21 | 전국 한 벌 |
...
| 상세주소 표시 두 벌 | `rnspbd_adrdc_mod.txt` · `rnspbt_adrdc_mod.txt` |
...
레이아웃 하나에 파일이 두 벌이다. **구조가 둘인 것이 아니라 값의 출처가 둘이다.**
...
**[전량 · 서울] 시군구용은 상세주소DB와 사실상 같은 자료다.** 533,103줄 대 533,102줄이고, PK 여덟 칸을 전량 대조해 128줄·129줄만 어긋났다. **적재는 한 벌만 한다.**

**[전량 · 서울] 건축물대장은 완전히 다른 자료다.** 3,790,271줄로 일곱 배이고, 건물 136,321채 가운데 122,159채가 공동주택이다. **공동주택 호수가 여기 있다.**

**두 벌은 대상이 갈린다.** 서울에서 시군구용에만 있는 건물 80,790채, 대장에만 있는 건물 136,020채, 양쪽에 다 있는 건물 301채다.
...
**PK가 도로명주소관리번호와 건물관리번호 쌍이다.** 도로명주소 한 자리에 건물관리번호가 여럿 붙는다는 구조적 증거다.
...
**건물관리번호가 없다.** 주소 한 자리를 가리킨다. 아파트 단지가 한 줄이다.
...
**주소 한 자리를 가리키는 식별자는 도로명주소관리번호 26이다.**
...
**원인은 행정구역 개편이다.** **건물관리번호는 부여 시점의 코드를 보존하고 법정동코드 칸은 현재값이라 갈린다.**
...
건물·도로명·주소자리·관련지번·상세위치다. **「이력 대상을 건물과 도로명 둘로 좁힌다」는 안은 자료로 성립하지 않는다.**
...
| 두 벌을 나눠 배포하는 근거 | 가이드가 테이블 이름만 적고 이유를 적지 않는다. **[확인 필요]** |
