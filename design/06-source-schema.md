...
**사용여부·상태 칸이 없다.** 살아 있는 건물만 담긴 스냅샷이라 폐지 건은 아예 빠져 있다.
...
**[전량] 2세트의 PK는 서로 겹치지 않는다.** 가이드가 PK로 표시한 다섯 칸(시군구코드·동일련번호·층일련번호·호일련번호·호접미사일련번호)을 2026년 7월 전체분에서 시도별로 대조했다. 시군구용 3,238,845개와 건축물대장 19,762,295개 가운데 겹치는 키가 0개다. 한 테이블에 함께 담아도 키가 부딪히지 않는다.
...
**폐지는 전체분에서 통째로 빠진다.** 전체분이 살아 있는 것만 담는 스냅샷이라는 사실과 맞는다.
...
| 2세트를 나눠 배포하는 근거 | 가이드가 테이블 이름만 적고 이유를 적지 않는다 |
