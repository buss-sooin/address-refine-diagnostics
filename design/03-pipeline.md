...
```
입력  청운동 100-4 자하문로30길 22
LotBranch    자하문로30길 22 · 자하문로30길 24
RoadBranch   자하문로30길 22
교집합       22  →  하나로 좁혀집니다. 지번 쪽 후보 22 · 24도 함께 돌려줍니다
```

**등급은 합친 결과로 정하고, 합치기 전의 결과를 표기별 후보로 함께 돌려줍니다.** 지번과 도로명이 함께 들어온 입력이면 `LotBranch`가 찾은 후보를 지번 쪽 후보로, `RoadBranch`가 찾은 후보를 도로명 쪽 후보로 이름표를 붙여 담습니다. 받는 쪽은 어느 표기가 무엇을 가리켰는지 봅니다. 주소 입력 팝업은 지번이 도로명주소 여럿을 가리킬 때 첫 후보를 자동으로 채우므로, 위 입력에서 지번 쪽에 24가 더 있었다는 사실이 결과에 남아야 합니다. 그래서 이 경우에 사유코드를 따로 두지 않습니다.

*돌려주는 값*: 건물 후보 집합과 버린 토큰 목록과 표기별 후보입니다. 어느 구현이 결과를 내지 못했는지가 여기에 담깁니다.
...
**폐지된 줄을 여기에서 제외합니다.** 폐지는 지우지 않고 표시만 남기므로 `ref` 테이블에 줄이 그대로 있습니다. 확인 쿼리가 `ref.building.stts`와 `ref.address.stts`가 `0`인 줄만, `ref.road`는 `use_yn`이 `0`인 줄만 받습니다. 상주 사전과 역조회 인덱스는 폐지된 값을 그대로 담고 있고 그것을 거르는 자리가 여기 하나입니다. 근거는 `04-source-data.md` 7절의 「캐시는 후보를 좁히는 데만 씁니다. 실재 판정은 DB가 합니다」입니다.
...
#### 기호는 사전과 입력을 같은 규칙으로 손질해 비교합니다

상주 사전은 원천 이름을 그대로 담으므로 `3.1만세로` · `2·28길` · `이대프리젠#4`처럼 기호가 붙은 이름이 기호째 들어갑니다. 입력에서만 기호를 지우면 이런 이름과 맞지 않습니다. 그래서 인덱스 기반 끊기와 같은 방식을 씁니다.

| | 무엇 | 예 |
|---|---|---|
| 사전 키 | 원천 이름에서 기호와 공백을 지운 형태 | `31만세로` · `이대프리젠4` |
| 사전 값 | 원천 이름 원본 | `3.1만세로` · `이대프리젠#4` |
| 조회 | 입력 토큰을 같은 규칙으로 손질해 키로 찾습니다 | `4·19로` → `419로` → `4.19로` |
| 돌려주는 값 | 사전 값. 입력에서 기호만 뺀 문자열을 돌려주지 않습니다 | |

**원문 그대로 맞는 이름을 먼저 찾고, 없을 때만 손질한 키로 찾습니다.** 정확 일치를 닮은 이름보다 앞에 두는 4-2절의 공통 흐름과 같은 차례입니다. [전량 · 전국] 건물이 있는 도로명 138,064개는 기호와 공백을 지워도 키가 겹치지 않습니다. 건물 이름 3칼럼 유일값 490,079개는 겹치는 키가 10,799개입니다(`#1` · `(1)` · `1`). 손질한 키가 이름 여럿을 가리키면 후보를 모두 열고 나머지 토큰과 `ExistenceVerifier`로 좁힙니다.

**입력 토큰 원문과 돌려준 사전 값이 다르면 교정입니다.** `TYPO_CORRECTED`를 붙이고 값에 「자하문@로 → 자하문로」처럼 원문과 고친 값을 적습니다. 기호가 원래 있는 이름인지 따로 판단하지 않습니다.

[전량 · 전국] 자리마다 원천에 나오는 기호는 이렇습니다. 앞 세 부분과 도로명과 번호 자리에서는 이 목록 밖의 기호가 실제 이름의 일부일 수 없으므로, 그 자리에 낀 기호는 항상 입력의 잘못입니다.

| 자리 | 원천에 나오는 기호 |
|---|---|
| 시도 · 시군구 · 읍면동 · 리 | 없습니다. 리 이름의 한자 괄호(`기암리(岐岩)`)는 같은 이름의 리를 구분하는 원천 표기이고 입력 대상이 아닙니다 |
| 도로명 | 숫자와 숫자 사이의 `.` · `·` · `ㆍ` 32개. 영문은 `APEC로` 1개 |
| 건물번호 · 지번 | 본번과 부번 사이 `-` |
| 건물 이름 · 상세주소 | 닫을 수 없습니다. 기호 50가지 이상과 한자와 로마 숫자가 원천 값에 있습니다 |
...
#### 시·도 줄임말은 가리키는 이름이 하나일 때만 코드 없이 확정합니다

`서울`·`경기`·`세종`처럼 시·도 자리에 단위 표기 없이 들어온 토큰은 층 조합 확인을 거친 뒤 그 토큰이 가리키는 이름의 수로 나눕니다.

| 층 조합 확인 뒤 남은 이름 | 처리 |
|---|---|
| 하나 | 코드를 붙이지 않고 확정합니다. `서울` → 서울특별시 |
| 둘 이상이었는데 `ExistenceVerifier`가 하나로 좁힘 | `UNIT_TOKEN_RESTORED`를 붙입니다. 검출 전략에서 확인 권장입니다 |
| 둘 이상이 끝까지 남음 | 후보 여럿입니다 |

「세종」은 인덱스에서 세종특별자치시와 도로명 세종로 · 세종대로를 함께 가리키지만, 뒤에 도로명과 건물번호가 오면 층 조합이 세종특별자치시 하나만 남깁니다. 시·도를 줄여 쓰는 것은 흔한 표기라서, 가리키는 이름이 하나인데도 코드를 붙이면 검출 전략에서 흔한 입력마다 확인 권장이 붙어 경고가 뜻을 잃습니다.

**어느 경우든 돌려주는 주소는 정식 표기입니다.** `서울`로 들어와도 `서울특별시`로 완성해 돌려줍니다. 코드가 없어도 입력 원문은 `svc.request`에 남으므로 무엇을 무엇으로 바꿨는지 기록에서 볼 수 있습니다.

시군구 층 아래에서 복원한 것에는 그대로 `UNIT_TOKEN_RESTORED`를 붙입니다. `서울종로자하문로96`은 「종로」를 복원했으므로 코드가 붙고, 채운 값에는 「종로 → 종로구」만 적습니다.
...
팝업이 지번↔도로명 일대다에서 첫 후보를 자동으로 채운 입력은 확인 권장으로 내지 않습니다. 합친 결과로 등급을 정하고, 지번 쪽에서 나온 나머지 후보를 표기별 후보로 함께 돌려줍니다. 0-1절 ② `CandidateMerger`에 있습니다.
...
수정 제안에는 처음 0이 된 층의 `NOT_FOUND` 코드를 붙입니다. 실패와 같은 코드이고, 등급과 제안 후보가 둘을 구분합니다. `서울특별시 만안구 중앙로 153`은 `SGG_NOT_FOUND`에 `서울특별시 양천구 중앙로 153`을 제안합니다.
...
→ **표기별 후보는 실무에서 겪은 사례로 만든 장치가 아니라, 구조상 생길 수 있는 문제를 미리 드러내려고 둔 장치입니다.**
...
서비스가 실행되는 동안 배치가 대응 사전을 스스로 채웁니다. 시간이 갈수록 대응 사전이 아는 범위가 넓어집니다.
...
두 테이블 모두 `ref`에 둡니다. 배치가 채우고 온라인 처리는 수정하지 못합니다.
...
ExistenceVerifier가 후보를 전부 지웠습니다
 → 주소 대응 사전을 조회합니다        있으면 새 등록주소로 잇습니다
 → 없으면 못 찾은 것으로 남깁니다
```

외부 주소 서비스는 조회하지 않습니다.
...
RoadBranch    →  「상무중앙로 110」으로 시군구를 역조회합니다
...
| 흐름 | `RefineOrchestrator` | 진입점이 `AddressRefiner` 인터페이스로 호출합니다 |
...
| 자원 포트 | 사전 조회 · 참조 조회 · 설정 조회 | 코어 안에서 저장소를 조회합니다 |
...
flowchart LR
  ...
  class REQ,RES,SVC,AC,AD,AR,ORCH,S1,S2,S3,S5,S6,S7,S8,B1,B2,B3,P1,P2,X1,X2,X3 m
  class S4,T1,T2,T3 i
...
파선 테두리는 인터페이스를 뜻합니다. `AddressBranch` 하나와 자원 포트 3개가 여기에 해당합니다. 나머지 도형 규약은 `01-scope.md` 6-1절과 같습니다.
...
| 포트와 어댑터(Ports and Adapters) | 자원 포트 3개 | 저장소 구현이 바뀌어도 코어를 고치지 않습니다 |
