...
교집합       22  →  하나로 좁혀지지만 JIBUN_AMBIGUOUS를 붙입니다
```

**지번 쪽 후보가 2개 이상인데 교집합이 1개가 되면 확인 권장으로 냅니다.** 사유코드는 `JIBUN_AMBIGUOUS`이고 사유 종류는 정보 부족입니다. 주소 입력 팝업은 지번이 도로명주소 여럿을 가리킬 때 첫 후보를 자동으로 채우므로, 이런 입력은 사용자가 고른 것이 아니라 팝업이 채운 값일 수 있습니다. 교집합에 들지 못한 지번 쪽 후보를 값으로 함께 돌려줍니다.
...
#### 시·도 줄임말에는 복원 코드를 붙이지 않습니다

`서울`·`경기`·`세종`처럼 시·도 층에서 단위 표기를 뗀 이름이 인덱스로 맞으면 `UNIT_TOKEN_RESTORED`를 붙이지 않고 확실한 성공으로 냅니다. 시·도를 줄여 쓰는 것은 흔한 표기이고, 시·도 이름끼리는 단위 표기를 떼어도 겹치지 않아 잘못 복원될 여지가 없습니다. 복원 코드를 붙이면 검출 전략에서 흔한 입력마다 확인 권장이 붙어 경고가 뜻을 잃습니다.
...
**응답 코드는 18개이고 정본은 `design/standard-dict/사유코드정의서.csv`입니다.** 정의서는 코드마다 그 코드를 내는 지점, 사유 종류, 등급, 근거를 담습니다. 코드를 더하는 것은 열려 있고, 빼는 것은 과거 기록이 뜻을 잃으므로 설계 논의를 거칩니다.
