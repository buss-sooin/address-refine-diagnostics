...
### 외부 서비스: 정제 코어는 호출하지 않습니다

정제 코어는 카카오 같은 외부 주소 서비스를 호출하지 않습니다. 정제 도중에 주소의 일부를 외부에 맡기지 않습니다.

외부 서비스는 정제 전체를 교체하는 확장 인터페이스의 다른 구현으로만 붙일 수 있습니다. 지금 만드는 구현은 정제 코어 하나이고, 외부 서비스 구현은 만들지 않습니다. 확장 인터페이스는 `07-interface.md` 4절에 있습니다.
...
GIS 모듈은 위 도식에 없습니다. 정제 흐름 밖에 있기 때문입니다.
...
측정 방법은 `05-expected-results.md` 3-4절에 있습니다.
...
자료명과 좌표계와 배포 단위와 연결 키는 좌표를 넣기로 정하는 시점에 그 자료의 가이드에서 읽습니다.
