...
| 정제요청 | 들어온 입력 한 건 | |
| 정제결과 | 들어온 입력에 돌려준 답 | 판정, 확정 건물, 돌려준 주소 텍스트 |
| 정제사유 | 답에 붙은 사유 하나 | 정제결과 한 건에 여러 건이 붙습니다. 버린 조각과 채운 값과 고친 값을 담습니다 |
| 적재실행 | 적재 진입점을 한 번 호출한 기록 | 폴더 하나가 한 줄입니다 |
| 적재파일 | 그 실행이 읽은 파일 한 개 | 적재한 파일과 건너뛴 파일을 함께 담습니다 |

**개념 단계의 엔터티는 8개입니다.** 논리 단계에서 「등록주소」·「관련지번」·「상세주소찾기」·「설정」·「정제사유」·「도로명읍면동」·「이름대응」·「주소대응」·「적재실행」·「적재파일」이 더해져 18개가 됩니다. 자세한 내용은 5절에 있습니다.
...
erDiagram
    ...
    정제사유 {
        string 사유순번
        string 사유코드
        string 원본값 "버린 조각 · 고치기 전 값"
        string 결과값 "채운 값 · 고친 값"
    }
```
...
폐지 줄에도 값이 다 들어옵니다. 월변동분 건물 폐지 6,469줄에서 전부 비는 칼럼은 6개이고 나머지 25칼럼이 채워져 왔습니다.

**폐지 표시를 하는 테이블이 3개입니다.** 건물과 도로명과 등록주소이고, 관련지번과 상세위치는 물리 삭제합니다. 일 반영과 월 대사가 같은 갈래를 따릅니다. 근거와 흐름은 `04-source-data.md` 8-2절과 8-3절에 있습니다.

상태를 담는 칼럼은 테이블마다 다릅니다. 도로명은 원천이 주는 `use_yn`을 그대로 쓰고, 건물과 등록주소는 우리가 만드는 `stts`를 씁니다. 원천에 그 칸이 없기 때문입니다.

**폐지관측일자는 말소일자가 아닙니다.** 원천의 말소일자는 전체분과 변동분 전량이 빈 값이라 언제 폐지됐는지를 주지 않습니다. 그래서 담을 수 있는 것은 우리가 그 폐지를 처음 본 날뿐이고, 이름도 그렇게 짓습니다. 값은 배치 실행일이 아니라 자료 세트의 기준일로 채웁니다. 실행일을 쓰면 과거 전체분을 소급 적재할 때 모든 줄이 같은 날짜가 되어 값이 사라집니다.
...
현재 상태는 다른 칼럼이 담습니다. 건물과 등록주소는 `stts`, 도로명은 `ref.road.use_yn`입니다. 폐지를 처음 관측한 날은 세 테이블 모두 `abl_obsrvn_ymd`가 담습니다. 갱신 배치는 삭제·삽입 건수와 새로 만든 대응 줄 수를 지표로 남깁니다.

전체분만으로는 대응이 한 줄도 나오지 않습니다. 비교할 이전 스냅샷이 없기 때문입니다. 대응은 서비스가 운영되는 동안 매달 쌓입니다.
...
**이력은 이미 수행한 정제를 나중에 그대로 꺼내 업무에 쓰기 위해 남깁니다.** 그래서 2가지를 남깁니다.

하나는 그때의 답 자체입니다. 확정 건물, 판정, 돌려준 주소 텍스트와 우편번호, 확정 동·층·호, 후보 수, 답에 붙은 사유(버린 조각 · 채운 값 · 고친 값)입니다. `ref`는 매달 현재 값으로 바뀌므로 답을 `ref`에서 다시 만들지 않고 값으로 복사해 둡니다.

다른 하나는 그 답을 다시 만드는 데 필요한 입력입니다. 주소 문자열, 우편번호, 전략구분, 설정, 그때 쓰인 자료 시점 5가지입니다. 자료 시점은 상주 사전을 마지막으로 갈아 끼운 적재실행입니다. 정제 코어는 LLM도 외부 호출도 없어 이 5가지가 같으면 같은 과정을 거쳐 같은 답을 냅니다. 테스트 코드는 이 입력으로 엔진을 다시 실행해 과정을 복원합니다.

**후보 목록은 남기지 않습니다.** 응답마다 후보를 모두 쌓으면 기록이 응답보다 커집니다. 후보는 위 입력으로 다시 만들고, 기록에는 후보 수만 남깁니다. 자료 시점이 바뀐 뒤에는 다시 만든 후보가 그때와 다를 수 있으며, 후보 수가 그 차이를 드러냅니다.
...
이 기록은 적재 판단에 관여하지 않습니다. 적재가 「현재 값으로 만듭니다」를 따르므로 같은 폴더를 두 번 적재해도 결과가 같습니다. 기록을 보고 이미 적재한 파일을 건너뛰게 만들면, 기록이 틀렸을 때 적재도 함께 틀립니다. 상세한 내용은 `04-source-data.md` 11-6절에 있습니다.
...
| 적재실행·적재파일 테이블 | 5-1절 (논리). 3절에 근거가 있고 규칙은 `04-source-data.md` 11-6절에 있습니다 |
...
| 정제요청과 정제결과를 전부 남길지 | 요청마다 남깁니다. 후보 목록은 남기지 않습니다. 보관 기간은 이번 단계에서 다루지 않습니다 |
| 조회에 쓸 인덱스 | 6-5절 (물리) |
...
개념 모델의 엔터티 8개에 「등록주소」·「관련지번」·「상세주소찾기」·「설정」·「정제사유」·「도로명읍면동」·「이름대응」·「주소대응」·「적재실행」·「적재파일」을 더하면 논리 모델의 엔터티는 모두 18개입니다. **이 절에서는 자료형과 길이를 정하지 않습니다.**
...
| 도로명의 이름·상위도로명번호·사용여부 | 제2정규형. 식별자가 도로명코드와 읍면동일련번호 두 속성인데, 이 세 속성은 앞의 도로명코드 하나로 정해집니다 | 「도로명읍면동」 엔터티를 새로 만들고, 읍면동 속성과 고시일자를 그쪽으로 옮깁니다 |
| 적재 실행 로그의 배치종류·자료연월·자료구분 | 제3정규형. 파일 행에 담으면 적재실행 식별자가 이 값들을 정합니다 | 「적재실행」과 「적재파일」로 나눕니다 |

원천 371,958행은 도로명코드 174,253개로 묶이며, 한 도로명코드 안에서 이름이 서로 다른 경우는 0건입니다. 원천이 도로명코드마다 대표 행을 하나씩 제공하고 있으므로, 도로명읍면동을 나누는 것은 원천 구조를 바꾸는 일이 아니라 섞여서 들어온 데이터를 원래대로 되돌리는 일입니다.

정제요청의 들어온경로 칼럼도 함께 뺐습니다. 처리 흐름 어디에서도 읽지 않는 칼럼이었기 때문입니다. 이 칼럼이 맡던 역할은 전략구분 칼럼과 설정식별자 칼럼이 대신합니다.

대응 엔터티 2개는 정규형을 지킵니다. 이름대응은 단계, 전명, 대응코드 세 속성이 전체 키이고, 현재명은 이 세 속성 전체에 종속합니다. 주소대응은 옛 등록주소 하나가 키이고, 현재 등록주소와 건물관리번호는 이 키에 종속합니다. 다형 참조도 없습니다. 이름과 등록주소를 한 테이블에 담으면 코드가 10자리와 12자리로 섞이므로 테이블을 나눴습니다.
...
식별 관계로 그린 곳은 7곳입니다. 등록주소와 건물, 등록주소와 관련지번, 지번과 관련지번, 등록주소와 상세주소찾기, 정제요청과 정제결과, 정제결과와 정제사유, 적재실행과 적재파일입니다.
...
정제사유 = 답에 붙은 사유코드마다 한 줄. 버린 조각 · 채운 값 · 고친 값을 함께 적는다
확실 + 사유가 함께 서면 건물판정을 확인권장으로 낸다
```

사유를 별도 엔터티로 두는 이유는 한 답에 사유가 여럿 붙기 때문입니다. 지번, 참고항목, 우편번호, 행정동이 한 입력에서 동시에 버려질 수 있고, `UNIT_TOKEN_RESTORED`와 `TYPO_CORRECTED`가 한 입력에 함께 붙을 수도 있습니다. 한 칼럼에 이어 붙이면 「지번만 버린 건이 몇 건인가」를 셀 수 없습니다.

한 줄에 담는 값은 사유코드에 따라 정해집니다. `PART_DISCARDED`는 원본값에 버린 조각을 적고, 채운 값을 알리는 코드 4개는 원본값에 원문(없으면 빈 값)을, 결과값에 채우거나 고친 값을 적습니다. 실패와 후보 여럿의 코드는 값을 비웁니다. 어느 값을 적는지가 사유코드로 정해지므로 처리 구분 칼럼을 따로 두지 않습니다.

버린 조각의 사유 코드는 하나만 둡니다. 종류마다 코드를 만들면 코드 목록이 계속 늘어나고, 응답을 받는 쪽도 그때마다 코드 목록을 갱신해야 합니다.
...
**칼럼 123개를 전부 대조했습니다.** 물리명을 이루는 단어 81개 가운데 75개가 공통표준단어이고 6개가 프로젝트 신규 단어입니다.
...
| 단어 | 약어 | 표준에 없는 이유 |
|---|---|---|
| 동(棟) | `BLK` | 표준용어 「행정동명 = `DONG_NM`」·「행정동코드 = `DONG_CD`」가 이미 `DONG`을 씁니다. 처음 승인은 `DONG`이었으나 한 약어가 洞·棟 두 뜻을 갖게 되어 `BLK`로 확정했습니다 |
| 호 | `HO` | 표준단어 3,284개에 홑글자 「호」가 없습니다 |
| 접미사 | `SFX` | 등재 없음. 영어 관용 축약 |
| 전 | `BFR` | 「이전 = `BFR`」의 축약을 빌립니다 |
| 오타 | `TYPO` | 등재 없음 |
| 후보 | `CAND` | 등재 없음. 「후보자 = `CNDD`」·「조건 = `CND`」와 겹치지 않습니다 |
...
대응 테이블 2개의 칼럼도 신규 단어 없이 모두 확정했습니다.
...
적재 실행 로그 2개도 신규 단어 없이 모두 확정했습니다. 칼럼 17개를 공통표준단어 16개의 조합으로 표기했습니다. 쓴 단어는 적재 `LOAD` · 실행 `EXCN` · 배치 `BTCH` · 자료 `DATA` · 호출 `CLOT` · 시작 `BGNG` · 종료 `END` · 결과 `RSLT` · 처리 `PRCS` · 제외 `EXCL` · 시도 `CTPV` · 대상 `TRGT` · 연월 `YM` · 건수 `NOCS` · 원천 `SOU` · 파일 `FILE`입니다.
...
폐지 상태를 담는 칼럼 4개도 신규 단어 없이 확정했습니다.

| 논리명 | 물리명 | 단어 조합 근거 |
|---|---|---|
| 상태 | `stts` | 상태 `STTS` |
| 폐지관측일자 | `abl_obsrvn_ymd` | 폐지 `ABL` + 관측 `OBSRVN` + 일자 `YMD` |

도메인은 기존 `일자D8`을 그대로 씁니다.
...
| 접수일시 · 적용일시 · 시작일시 · 종료일시 | `timestamptz` | 시간대를 함께 담습니다 |
...
| `abl_obsrvn_ymd` | `date` | | 폐지관측일자. 우리가 만듭니다 |
...
**상태 칼럼을 `ref.building`과 같은 이름으로 둡니다.** 등록주소도 폐지되면 지우지 않고 표시만 남기기 때문입니다. 지우는 쪽을 골랐다면 `ref.address_lot`과 `ref.unit_lookup`의 줄이 가리킬 대상을 잃고, 그 등록주소를 바라보던 건물이 주소 없는 상태가 됩니다. `ref` 안쪽에 외래키를 걸지 않으므로 DB가 그 상태를 막아 주지도 않습니다.
...
대표를 `rprs`로 씁니다. 표준단어가 「대표 = `RPRS`」이기 때문입니다.
...
| `crt_src` | 생성출처 | `varchar(1)` | `CHECK (crt_src IN ('B','P'))` · B 배치비교 · P 과거연동 |
...
| `crt_src` | 생성출처 | `varchar(1)` | `CHECK (crt_src IN ('B','P'))` |
...
**설정을 `cfg`가 아니라 `stng`로 씁니다.** 표준단어가 「설정 = `STNG`」이기 때문입니다.

「오타 = `TYPO`」와 「후보 = `CAND`」가 프로젝트 신규 표준단어이고, 나머지는 공통표준단어의 조합입니다.
...
| `orgnl_dmnd` | `jsonb` | |
| `zip_vl` | `text` | 우편번호값. 들어온 그대로 적습니다 |
| `strtg_se` | `varchar(1)` | NOT NULL · `CHECK (strtg_se IN ('C','D'))` · C 교정 · D 검출 |
| `stng_idntfr` | `bigint` | FK → `svc.config` |
| `load_excn_idntfr` | `bigint` | FK → `svc.load_run` · 상주 사전을 마지막으로 갈아 끼운 적재실행 |
...
**우편번호값도 길이를 제한하지 않습니다.** 앞자리 0이 빠진 `3047`이나 엉뚱한 값도 들어온 그대로 남겨야 재현이 됩니다. 원본요청은 감사 기록이라 조회하지 않으므로, 재현에 쓰는 우편번호는 이 칼럼이 따로 담습니다.

적재실행식별자는 그 요청을 처리할 때 쓰인 자료 시점입니다. 같은 입력이라도 이름대응 사전과 `ref`의 상태에 따라 답이 달라지므로 이 칼럼이 없으면 결과를 재현하지 못합니다.
...
| `bldg_mng_no` | `varchar(25)` | 확정 건물. 확정하지 못했으면 빈 값 |
| `cfmtn_addr` | `text` | 확정주소. 돌려준 주소 텍스트를 값으로 복사합니다 |
| `cfmtn_zip` | `varchar(5)` | 확정우편번호. 돌려준 우편번호 |
...
| `cand_nocs` | `integer` | 후보건수. `ExistenceVerifier`를 통과한 건물 후보 수 |

사유코드는 이 테이블에 두지 않고 `svc.result_reason`에 둡니다. 한 답에 사유가 여럿 붙기 때문입니다. 사유종류 칼럼을 두지 않습니다. 사유코드가 정해지면 사유종류도 따라 정해지고, 그 대응은 코드 마스터가 담습니다.

**판정 두 칼럼의 코드 문자는 표준코드정의서가 정본입니다.** 규약은 「우리가 만드는 값은 뜻의 영문 첫 글자를 대문자로 쓰고 칼럼 안에서만 유일합니다」이고, 원천이 주는 값은 원천 문자를 그대로 받습니다.

미입력과 해당 없음의 코드 문자를 구분했습니다. 두 값 모두 None이 될 수 있어, 사용자가 입력하지 않은 경우는 `E`(Empty)로, 그 건물에 상세주소 자체가 없는 경우는 `N`(None)으로 둡니다.

**확정주소와 확정우편번호를 `ref`에서 다시 만들지 않고 복사해 둡니다.** 확정 동·층·호를 복사하는 것과 같은 이유입니다. `ref`가 갱신되어 이름이 바뀌면(인천 서구가 서해구와 검단구로 나뉜 것처럼) 그때 돌려준 텍스트를 다시 만들 수 없습니다.

후보건수는 후보 목록 대신 남깁니다. 후보 여럿과 상한 초과로 목록을 비운 경우에도 개수는 남고, 다시 실행한 결과의 후보 수와 견주어 자료 시점이 바뀌었는지 알 수 있습니다.

#### `svc.result_reason`: 정제사유

| 칼럼 | 자료형 | 제약 |
|---|---|---|
| `dmnd_idntfr` | `bigint` | PK1 · FK → `svc.result` |
| `rsn_sn` | `smallint` | PK2 · 사유순번 |
| `rsn_cd` | `varchar(30)` | NOT NULL · 사유코드. 목록은 사유코드정의서가 담습니다 |
| `orgnl_vl` | `text` | 원본값. 버린 조각이나 고치기 전 원문 |
| `rslt_vl` | `text` | 결과값. 채우거나 고친 값 |

**한 답에 사유가 여럿 붙습니다.** 지번과 참고항목과 우편번호가 동시에 버려질 수 있고, 복원과 교정이 함께 일어날 수 있습니다. 이것을 한 칼럼에 이어 붙이면 「지번만 버린 건이 몇 건인가」를 셀 수 없습니다.

어느 값 칸을 채우는지는 사유코드가 정합니다. 5-8절에 있습니다.
...
**폴더 하나가 한 줄에 대응합니다.** 연월과 구분은 경로에서 읽고, 배치구분은 배치를 호출할 때 받습니다. 경로에 「무슨 배치인가」가 담겨 있지 않기 때문입니다. 근거는 `04-source-data.md` 11-6절에 있습니다.

실행결과 코드에 실행 중이라는 값을 둡니다. 시작할 때 넣고 끝날 때 갱신합니다. 이 값이 같은 폴더를 동시에 두 번 적재하는 것을 막습니다. 사전 갈아 끼우기에서 「이미 실행 중입니다」를 돌려주는 규칙과 같은 구조입니다.

호출구분을 두는 이유는 `04-source-data.md` 7-1절과 짝을 맞추기 위해서입니다. 사전 갈아 끼우기에서 「자동인가 수동인가」를 지표로 남기기로 정해 두었고, 같은 운영 진입점에 붙는 적재가 그것을 남기지 않을 이유가 없습니다.
...
폐지 건을 지우지 않기로 했으므로, 유효한 줄만 담는 부분 인덱스를 따로 둘 수 있습니다. 조건은 `WHERE stts = '0'`이고 대상은 `ref.building`과 `ref.address`입니다. 부분 인덱스는 오라클에는 없는 기능입니다.
...
`svc` 안쪽 외래키는 5개입니다. 정제요청 → 설정, 정제요청 → 적재실행, 정제결과 → 정제요청, 정제사유 → 정제결과, 적재파일 → 적재실행입니다.
...
erDiagram
    ...
    svc_result     ||--o{ svc_result_reason  : "dmnd_idntfr · FK"
    svc_load_run   ||..o{ svc_request        : "load_excn_idntfr · FK"
    ...
    ref_name_map {
        ...
        varchar1  crt_src "B 배치비교 P 과거연동"
    }
    ref_addr_map {
