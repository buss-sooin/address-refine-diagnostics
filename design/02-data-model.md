...
이 기록은 적재 판단에 관여하지 않습니다. 적재가 「현재 값으로 만듭니다」를 따르므로 같은 폴더를 두 번 적재해도 결과가 같습니다. 기록을 보고 이미 적재한 파일을 건너뛰게 만들면, 기록이 틀렸을 때 적재도 함께 틀립니다. 상세한 내용은 `04-source-data.md` 14-6절에 있습니다.
...
| 적재실행·적재파일 테이블 | 5-1절 (논리). 3절에 근거가 있고 규칙은 `04-source-data.md` 14-6절에 있습니다 |
...
**[확인 필요]** 대표 줄이 없는 도로명코드가 하나 있습니다(174,253 대 174,252). 줄이 서른 개인 코드도 아직 확인하지 않았습니다. 2가지 모두 적재 배치를 만들 때 해결합니다.
...
> **[확인 필요: 한 등록주소의 건물이 서로 다른 새 자리로 나뉘는 경우가 있는가]** 있으면 기본키에 건물관리번호를 더해야 합니다. 여러 달치 변동분을 비교해야 답이 나옵니다.
...
**폴더 하나가 한 줄에 대응합니다.** 연월과 구분은 경로에서 읽고, 배치구분은 배치를 호출할 때 받습니다. 경로에 「무슨 배치인가」가 담겨 있지 않기 때문입니다. 근거는 `04-source-data.md` 14-6절에 있습니다.
...
전국으로 넓힐 때 다시 검토합니다. [확인 필요: 도로명주소관리번호 26자리의 앞자리 구조]
...
- [ ] 한 등록주소의 건물이 서로 다른 새 자리로 나뉘는 경우가 있는가(있으면 `ref.addr_map` 기본키에 건물관리번호를 더합니다)
- [ ] 건물관리번호 안의 지번이 현재 지번과 어긋나는 원인 (표본 9%)
- [ ] 상세주소찾기 테이블의 전국 합계 줄 수
- [ ] 상세주소 표시 2세트의 PK 여덟 칼럼이 서로 겹치는가
- [ ] 행정동 ↔ 법정동 대응표를 어디서 얻을지
- [ ] 도로명주소관리번호 26자리의 앞자리 구조
- [ ] `autovacuum` 고정 건수 5만이 맞는 값인가
- [ ] 폐지 건물 비율

「도로명이 바뀐 대응 관계를 어디서 얻을지」가 닫혔습니다. 밖에서 얻지 않고 갱신 전후를 견줘 우리가 만듭니다.

「`svc.config` 세 칼럼의 물리명」이 닫혔습니다. 2026-08-28 표준 대조로 정했습니다.
