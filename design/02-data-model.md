...
도메인이 하나 늘었습니다. 자료연월이 `varchar(6)`이라 `연월V6` 도메인을 새로 정의했습니다. 도메인 수는 25개에서 26개가 됩니다.

폐지 상태를 담는 칼럼 4개도 신규 단어 없이 확정했습니다 (2026-09-18).
...
「폐지 = `ABL`」이 공통표준단어에 있어 프로젝트 신규 단어는 6개 그대로이고, 쓰는 단어 수만 83개에서 84개가 됐습니다. 도메인은 기존 `일자D8`을 그대로 씁니다. `ref.name_map`과 `ref.addr_map`이 이미 「관측일자 `obsrvn_ymd`」를 같은 뜻으로 쓰고 있어 이름의 결도 맞습니다.
...
| 규약의 예외가 있는가 | 3가지입니다. `ref.building.stts`와 `ref.address.stts`는 원천 `use_yn`과 같은 뜻이라 `0`·`1`을 쓰고, `ref.region.stp`는 `smallint`라 애초에 문자가 아닙니다 |
...
| 칼럼이 몇 개인가 | 121개입니다. 표준단어 84개 가운데 78개가 공통표준단어이고 6개가 프로젝트 신규 단어입니다 |
...
python3 gen_std_dict.py   # 표준사전 CSV 다섯    (칼럼 121 · 단어 84 · 도메인 26 · 코드 25 · 사유코드 17)
